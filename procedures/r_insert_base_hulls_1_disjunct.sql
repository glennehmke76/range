DROP PROCEDURE IF EXISTS r_insert_base_hulls_disjunct;
CREATE OR REPLACE PROCEDURE r_insert_base_hulls_disjunct(
  p_sp_id integer,
  p_hull_type varchar,
  p_alpha numeric,
  p_class integer
...
BEGIN
    ...
    WITH disjunct AS
      ...
      FROM sightings
      WHERE
        (class_specified IS NULL OR class_specified > 0)
...
CALL r_insert_base_hulls_disjunct(
    p_sp_id := 402,
    p_hull_type := 'alpha',
    p_alpha := 2.5,
    p_class := 0
