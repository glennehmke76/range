...
CREATE TABLE tmp_region AS
  SELECT
    ...
    geom
--     ST_Union(ST_Intersection(region_sibra.geom,region_continental.geom)) AS geom
  FROM region_sibra_simple
--   JOIN region_continental ON ST_Intersects(region_sibra.geom,region_continental.geom)
--   GROUP BY
--     region_sibra.id;
;
CREATE INDEX IF NOT EXISTS idx_tmp_region_id
ON tmp_region (id);
...
WITH hull_sightings AS
  ...
  FROM
      ...
      WHERE
        range_region_rr.regionalisation = 'sibra'
  ...
  FROM
      (SELECT
        ...
        ST_Collect(tmp_region_sightings.geom),
          2.5, false)) AS hull
      FROM tmp_region_sightings
      JOIN range_region_rr ON ST_Intersects(tmp_region_sightings.geom, range_region_rr.geom)
      WHERE
        range_region_rr.regionalisation = 'basins_clipped'
        AND range_region_rr.rr_percentile >= 20
        -- exclude point geoms from coming through
      HAVING
        Count(DISTINCT tmp_region_sightings.geom) >2
      )hulls
;
...
In the end specify core region manually based on expertise and then run alpha shape...